...
<div class="container">
    ...
    <div class="addgoals col-md-4">
    	<form role="form"action="#" method="POST">
    		<legend>Doelpunten</legend>
    		<div class="form-group">
    			<label for="speler" id="speler">Speler</label>
    				<?php 
    				$sql = "SELECT spelers.id, spelers.voornaam, spelers.tussenvoegsel, spelers.achternaam, teams.naam FROM spelers JOIN teams ON spelers.team_id = teams.id ORDER BY teams.naam";
    				if ( !$query = mysqli_query($con, $sql) ) {
    					echo 'Kan spelerslijst niet ophalen';
    					exit();
    				}
    				?>
    				...
    				<?php 	
    					while ( $row = mysqli_fetch_assoc($query) )
    					{
    						echo '<option value="' . $row['id'] . '">' . $row['id'] . ' | '  . $row['naam'] . ' - ' . $row['voornaam'] . ' ' . $row['tussenvoegsel'] . ' ' . $row['achternaam'] . '</option>'; 
    					}
    				 ?>			
    				 </select>
    				 <button onclick="scoordersToevoegen()">scoorders toevoegen</button>			
    		</div>
    	</form>
    	<ul id="scoredGoals">
